...
//modifico la tabla serial;
$serialp=$_POST["serialp"];
$serialu=$_POST["serialu"];
$letra=$_POST["letra"];
$query="update serial set nro=".$serialu.", letra='".$letra."' where nro=".$serialp.";"; //insertamos los nuevos valores
$db->Execute($query) or Error($query);
/*$query="update serial set lock='0' where nro=".$ultimo_ser.";"; //desbloqueamos la variable lock
ejecutar_query($query);*/

//funcionalida de guardar los datos de la nueva orden de produccion
//en la base de datos.
...
{switch($tipo)
 ...
 $sql="select razon_social from proveedor where id_proveedor=$prov";
 $res=$db->Execute($sql) or Error($sql);
 $prov=$res->fields['razon_social'];
 $query_comp="insert into componentes(id_configuracion,tipo,esp1,esp2,esp3,esp4,observaciones_componente,garantia,nombre_proveedor) values($id_conf,$tipo,'$marca','$modelo','$especificacion3','$especificacion4','$observ','$garantia','$prov')";
 ...
 //que es el que se acaba de insertar.
  ...
  $query_fact="insert into factura_componente (id_componente,nro_factura) values($id_comp,'$fact')";
...
$query_cl="insert into cliente_final (nombre,direccion,tel,comentarios,email) values('$nomb_cl','falta completar en el programa','','','')";
...
$u=$_ses_user['name'];
if ($lic!="")
$query_orden="insert into orden_de_produccion(id_ensamblador,id_cliente,id_configuracion,fecha_inicio,fecha_entrega,lugar_entrega,estado,cantidad,primera_maquina,ultima_maquina,aprobada,autor_orden,garantia,id_licitacion)
values($nro_ens,$cliente,$id_conf,'$f_ini','$f_ent','$l_ent',0,$cantidad,'$primer_ser','$ultimo_ser',0,'$u','$garantia',$lic)";
else
$query_orden="insert into orden_de_produccion(id_ensamblador,id_cliente,id_configuracion,fecha_inicio,fecha_entrega,lugar_entrega,estado,cantidad,primera_maquina,ultima_maquina,aprobada,autor_orden,garantia) 
values($nro_ens,$cliente,$id_conf,'$f_ini','$f_ent','$l_ent',0,$cantidad,'$primer_ser','$ultimo_ser',0,'$u','$garantia')";
$db->Execute($query_orden) or Error($query_orden);
db_tipo_res('a');
$query_orden="select max(nro_orden) from orden_de_produccion";
$resultado=$db->Execute($query_orden) or Error($query_orden);
$nro_orden=$resultado->fields["max"];

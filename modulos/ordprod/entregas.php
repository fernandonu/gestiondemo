...
$Author: mari $
$Revision: 1.19 $
$Date: 2006/08/28 21:58:29 $
*/

require_once("../../config.php");
echo $html_header;

variables_form_busqueda("entregas");

if ($cmd == "") {
	$cmd="pendientes";
	phpss_svars_set("_ses_entregas_cmd", $cmd);
}
...
$seleccion = Array ( "comentarios" => "id_entrega_estimada in (select id_entrega_estimada
                                        from licitaciones.comentarios_seguimientos
                                        ...
                                        union
                                        select id_entrega_estimada
                                        from licitaciones_datos_adicionales.comentario_prorroga
                                        join licitaciones_datos_adicionales.prorroga using (id_prorroga)
                                        where comentario ILIKE '%$keyword%')"
);
...
$filtro = Array (
   ...
   "fec_entrega" => "Entrega Renglon ",
   "comentarios" => "Comentarios",
   "subido_lic_oc.lugar_entrega" => "Lugar de Entrega",
   "facturas"=>"Nro de Factura"
);

$sql_tmp  = "SELECT licitacion.id_licitacion,entrega_estimada.nro,entrega_estimada.id_entrega_estimada,";
$sql_tmp .= " entidad.nombre as nombre_entidad,distrito.nombre as nombre_distrito,fec_entrega,";
...
$sql_tmp .= " LEFT JOIN (select max(log_renglones_oc.fecha_entrega) as fec_entrega,id_licitacion 
                         from licitaciones.renglon join renglones_oc using(id_renglon)
                         join log_renglones_oc using(id_renglones_oc)
                         join licitaciones.licitacion using(id_licitacion) 
                         group by id_licitacion) as entrega";
$sql_tmp .= " USING (id_licitacion) ";
...
$sql_tmp .= " LEFT JOIN (
              select licitaciones.unir_texto( facturas.nro_factura || text(' ')) as facturas,id_subir 
            from ( select  distinct (numeracion_sucursal.numeracion || text('-') || facturas.nro_factura) as nro_factura ,id_subir 
                   ...
                   where facturas.estado<>'a' ) as facturas 
            group by id_subir
            ) as f using (id_subir)";
/*
$sql_tmp .= " LEFT JOIN licitaciones.renglones_oc using(id_subir)";
$sql_tmp .= " LEFT JOIN facturacion.items_factura using (id_renglones_oc)";
$sql_tmo .= " LEFT JOIN facutacion.facturas using(id_factura)"
*/
if ($cmd=="pendientes") {
...
<table border=0 width=100% cellspacing=2 cellpadding=3 >
  <tr>
  <td colspan=8 align=left id=ma> <? echo "\n";?>
	<table width=100%>
	 <tr id=ma><? echo "\n";?>
  ...
  <tr>
      <td align="center" id=mo><a id=mo href='<? echo encode_link("entregas.php",Array('sort'=>1,'up'=>$up2,'page'=>$page,'keyword'=>$keyword,'filter'=>$filter))?>'><b>ID LIC</b></a></td>
      <td align="center" id=mo><a id=mo href='<? echo encode_link("entregas.php",Array('sort'=>7,'up'=>$up2,'page'=>$page,'keyword'=>$keyword,'filter'=>$filter))?>'><b>N° SEG</b></a></td>
      <td align="center" id=mo><a id=mo href='<? echo encode_link("entregas.php",Array('sort'=>2,'up'=>$up2,'page'=>$page,'keyword'=>$keyword,'filter'=>$filter))?>'><b>NRO OC</b></a></td>
      <td align="center" id=mo><a id=mo href='<? echo encode_link("entregas.php",Array('sort'=>9,'up'=>$up2,'page'=>$page,'keyword'=>$keyword,'filter'=>$filter))?>'><b>FACTURAS</b></a></td>
      <td align="center" id=mo><a id=mo href='<? echo encode_link("entregas.php",Array('sort'=>3,'up'=>$up2,'page'=>$page,'keyword'=>$keyword,'filter'=>$filter))?>'><b>VENCIMIENTO ENTREGA</b></a></td>
      <td align="center" id=mo><a id=mo href='<? echo encode_link("entregas.php",Array('sort'=>4,'up'=>$up2,'page'=>$page,'keyword'=>$keyword,'filter'=>$filter))?>'><b>CLIENTE</b></a></td>
      <td align="center" id=mo><a id=mo href='<? echo encode_link("entregas.php",Array('sort'=>5,'up'=>$up2,'page'=>$page,'keyword'=>$keyword,'filter'=>$filter))?>'><b>ENTREGA RENGLON</b></a></td>
      <td align="center" id=mo><a id=mo href='<? echo encode_link("entregas.php",Array('sort'=>6,'up'=>$up2,'page'=>$page,'keyword'=>$keyword,'filter'=>$filter))?>'><b>STATUS</b></a></td>
  </tr>
 ...
 while (!$res_query->EOF) {
  ...
  if ($cmd=='pendientes') {
    ...
    */
    ...
    $fecha=split("-",$fecha_finalizacion);
    $fecha_finalizacion_comparacion=date("d/m/Y",mktime(0,0,0,$fecha[1],$fecha[2]+10,$fecha[0]));
    //echo "$fecha_finalizacion -- $fecha_finalizacion_comparacion";
    if ($fecha_finalizacion)
       $comp=compara_fechas(fecha_db($fecha_vencimiento),$fecha_finalizacion_comparacion);
    ...
    $ref=encode_link('seleccionar_renglon_adj.php',array("id_entrega_estimada"=>$id_entrega_estimada,"licitacion"=>$id,"numero"=>$nro,"pagina_volver"=>'entregas.php',"oc"=>$oc,"cliente"=>$cliente,"vencimiento"=>$vencimiento));
  ...
  <tr <?=$atrib_tr?> >
    ...
    <td align="center" style="cursor:hand"><?=$res_query->fields['facturas'] ?></td>
    </a>
     <a href="<?=$ref?>">
    ...
    <td align="center" style="cursor:hand"><?=Fecha($res_query->fields['fec_entrega']) ?></td>

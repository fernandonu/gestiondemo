...
{
    $acceso="Todos";
    $comentario="Archivo de Orden de Producción";
    $fecha=date("Y-m-d H:i:s");
    $files_total=$_POST['select_filecount'];
    $id_p_e=$_POST['id_prod_esp'];
    $sacar="archivo[1]";
    ...
    for ($i=0; $i < $files_total ; $i++ )
    {
	    ...
	    if (!$error_msg) 
	    {
	    	...
	    	{
	         $sql="select nextval('foto_producto_id_foto_producto_seq') as idfile ";
	         $res=sql($sql) or $db->errormsg()."<br>";
	         $idfile=$res->fields['idfile'];
	         $q="INSERT INTO general.foto_producto
	              (id_foto_producto,id_prod_esp,nombre_archivo,comentario_foto,tamano,subido_usuario,subido_fecha,tipo) Values
...
<form name='form_archivos' action='guardar_foto.php' method=POST enctype='multipart/form-data'>
...
<tr>
  ...
  <td style="border:#E0E0E0;" colspan=2 align=center id=mo><font size=3><b>Foto del producto <?=$nombre_producto?></b></td>
</tr><tr>

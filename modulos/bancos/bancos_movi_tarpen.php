...
$_POST["idbanco"] = $_POST["Mov_Tarjetas_Pendientes_Banco"];
variables_form_busqueda("movi_tarpan",array("idbanco" => ""));
...
if ($_POST["Modificacion_Tarjeta_Guardar"]) {
    ...
    $idtipotar=$_POST['Modificar_Tarjeta_Tipo'];
    ...
    if ($fecha_deposito == "") {
        ...
        if (FechaOk($fecha_deposito)) {
            $fecha_deposito = "'".Fecha_db($fecha_deposito)."'";
        }
        else {
    ...
    elseif (!es_numero($importe_credito)) {
        Error("El Importe del Credito ingresado es inválido");
    }
...
if ($_POST["Modificar"]) {
	...
	if (es_numero($mod_numero)) {
		...
		echo "<form action=bancos_movi_tarpen.php method=post>\n";
...
if ($_POST["Actualizar"]) {
    ...
    $sql = "SELECT * FROM bancos.tipo_banco WHERE activo=1 order by nombrebanco";
    $result = $db->execute($sql) or die($db->ErrorMsg());
    echo "<select name=Mov_Tarjetas_Pendientes_Banco OnChange=\"document.forms[0].submit();\">\n";
   ...
   $orden = array(
			...
			"default_up" => "1",     //orden por defecto
...
//prefijo para los links de paginas siguiente y anterior
   ...
   $where_tmp = "bancos.tarjetas.fechacrédtar IS NULL ";
   if ($Banco!="todos")
       $where_tmp .= "AND bancos.tarjetas.idbanco=$Banco";
   else
       $where_tmp .= "AND tipo_banco.activo=1";
//   echo "$sql_tmp - $orden - $filtro - $link_tmp - $where_tmp";
   ...
   $SubTotal = 0;
	echo "</table>";
	echo "<table width='98%' align=center class='bordes'>";
	echo "<tr><td id=ma colspan=7>";// bordercolor='#000000'
	echo "<table id=ma border=0 width=100%><tr>\n";
	echo "<td align=left width=70%>Tarjetas Pendientes</td>\n";
	echo "<td align=right width=30%>$link_pagina&nbsp;</td>\n";
	echo "</tr></table>\n";
	echo "</td></tr>";
    echo "<tr id=mo>";//bordercolor='#000000'
    echo "<td align=center>&nbsp;</td>";
    echo "<td align=center><a id=ma href='".encode_link("bancos_movi_tarpen.php",Array('sort'=>1,'up'=>$up))."'>ID</a></td>";
    echo "<td align=center><a id=ma href='".encode_link("bancos_movi_tarpen.php",Array('sort'=>2,'up'=>$up))."'>Tarjeta</a></td>";
    echo "<td align=center><a id=ma href='".encode_link("bancos_movi_tarpen.php",Array('sort'=>3,'up'=>$up))."'>Fecha Depósito</a></td>";
    echo "<td align=center><a id=ma href='".encode_link("bancos_movi_tarpen.php",Array('sort'=>4,'up'=>$up))."'>Importe Depósito</a></td>";
    ...
    {
   		ob_clean();
   		excel_header("tar_pendientes.xls");
   		echo "<html>";
			echo "<style type=\"text/css\">\n";
   		require("../../lib/estilos.css");
   		echo "</style>\n";
    	echo "<table>\n";
	    echo "<tr><td align=center colspan=4><b>TARJETAS PENDIENTES</b></td></tr>\n";
	    echo "<tr><td>&nbsp;</td></tr>\n";
    	echo "<tr><td>&nbsp;</td><td colspan=3><b>Banco: </b>$banco_nbre</td></tr>\n";
    	if ($keyword)
    		echo "<tr><td>&nbsp;</td><td colspan=3><b>Palabra buscada:</b> '$keyword' <i>en ".($filtro[$filter]?"campo ".$filtro[$filter]:"Todos los campos")."</i></td></tr>\n";
	    echo "<tr><td>&nbsp;</td><td colspan=3><b>Registros encontrados: ".$result->recordcount()."</b></td></tr>\n";
	    echo "<tr><td>&nbsp;</td><td colspan=3><b>Total Pendiente: \$ $Total</b></td></tr>\n";
//	    echo "<tr><td>&nbsp;</td><td colspan=3><b>Total Busqueda: \$ $SubTotal</b></td></tr>\n";
	    echo "<tr><td>&nbsp;</td></tr>\n";
   ...
   while ($fila = $result->fetchrow()) {
	 ...
	 if ($download)
   {
    echo "<tr bgcolor=#000000 style='color:#E9E9E9;font-weight: bold;'><td colspan=4 align=center><b>Subtotal Búsqueda: \$ ".formato_money($SubTotal)."</b></td></tr>\n";   	
   	echo "</table>\n";
   	echo "<table><tr><td>&nbsp;</td></tr></table>\n";
   }
   else
   {
    echo "<tr><td colspan=7 align=center bgcolor=$bgcolor3><b>Subtotal Pendiente: \$ ".formato_money($SubTotal)."</b></td></tr>";
    echo "</table></form>\n";
   }
?>
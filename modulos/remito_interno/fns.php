...
function get_items($id_remito=false)
{
 ...
 if(!($id_remito===false))
 {
	 $query="SELECT * FROM remito_interno as r JOIN items_remito_interno as i on ".
...
function del_items($items,$id_remito)
{
	...
	$q= "delete from items_remito_interno where id_remito=$id_remito AND id_item in ($borrar)";
	if ($borrar!="")

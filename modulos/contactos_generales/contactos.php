...
//me fijo si ya existe un contacto con el mismo nombre si ya existe no lo inserto
   ...
   if ($resultado->RecordCount()==1)
               ...
               $campos="(nombre,tel,direccion,provincia,localidad,cod_postal,mail,fax,icq,observaciones)";
               $query_insert="INSERT INTO contactos_generales $campos VALUES ".
               "('$nombre','$tel','$direccion','$provincia','$localidad','$cod_postal','$mail','$fax','$icq','$observaciones')";
              ...
              //insertamos el id de entidad al que pertenece el contacto si no ersta ya en la BD
...
{$flag1=1;
 $flag2=1;
$db->StartTrans();
$query="update contactos_generales set nombre='$nombre',direccion='$direccion',cod_postal='$cod_postal',localidad='$localidad',provincia='$provincia',
...
//echo $query;
 if (sql($query))
   ...
   while(!$mod->EOF)
   {
   	if($_POST[$mod->fields['nombre']]==$mod->fields['id_modulo'])
    {
     $query_mod1="insert into modulos_contacto (id_contacto_general,id_modulo) values($select_contactos,".$mod->fields['id_modulo'].")";
      ...
      <td width="50%" align="center"><table width="99%" border="0" align="center" cellpadding="1" cellspacing="1" bgcolor=#E0E0E0>
         ...
         <td  nowrap><strong>Módulo</strong></td>
            ...
            <?//generacion de los checkboxes para los modulos
             ...
             while(!$mod->EOF)
             {?>
              <input type="checkbox" name="<?=$mod->fields['nombre']?>" value="<?=$mod->fields['id_modulo']?>"> <?=$mod->fields['nombre']?>
             <?
             $mod->MoveNext();
             }
            ?>

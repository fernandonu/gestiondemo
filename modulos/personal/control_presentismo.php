...
function muestra_tabla(obj_tabla, nro){
					...
					<tr>
						<td align=center>
							<?
								...
								if ($ubicacion){
									...
									if (($_POST["sel_ubicacion"])&&($_POST["sel_ubicacion"]!="Todas")){
										...
										$where_tmp.="and(d.nombre ilike '%Buenos Aires%')";
									}else{
										$where_tmp.="and(1=1)";
									}
								}
							...
							<select id="sel_ubicacion" name="sel_ubicacion">
							  <option value='Buenos Aires' <?if ($ubic=='Buenos Aires') echo 'selected'?>>Buenos Aires </option>
	...
	<table border=0 width=95% cellspacing=2 cellpadding=3 bgcolor='<? echo $bgcolor3; ?>' align=center>
		<tr>
			<input type="submit" name="stat_bsas" value="Buenos Aires"></input>
						...
						$asist = "select *
							from (
								...
								from personal.asistencia
									...
									left join licitaciones.distrito on (pcia_ubicacion=id_distrito)
...
?>
	...
	<?   
			...
			for ($i=0; $i<count($lista_nombres); $i++){
			 ...
			 if ($ubic=="") $ubic_personal="Buenos Aires";

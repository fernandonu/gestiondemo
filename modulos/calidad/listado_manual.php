...
$Author: mari $
$Revision: 1.18 $
$Date: 2006/12/27 18:28:52 $
*/

require_once("../../config.php");

$distrito=$_GET["distrito"] or $distrito=$parametros["distrito"];

if ($distrito=="bs_as"){
   phpss_svars_set("_ses_global_manuales_distrito","bs_as");
}
elseif ($distrito=="san_luis"){
   phpss_svars_set("_ses_global_manuales_distrito","san_luis");
 }
...
if(!permisos_check("inicio","permiso_barra_pendientes_manual"))
{$datos_barra = array(
     				array(
						"descripcion"	=> "Válidos",
						"cmd"			=> "validos",
                        "extra"=>array("distrito"=>$_ses_global_manuales_distrito),
						),
				 );
}
else
...
if ($_ses_global_manuales_distrito=="bs_as") $id_distrito=2;
           elseif($_ses_global_manuales_distrito=="san_luis")
                                             $id_distrito=1;
...
if($cmd=="pendientes")
    {
     ...
     $contar="select count(*) from manual where ((estado=0 or estado=1) and historial=0) and id_distrito=$id_distrito";
    }
    elseif($cmd=="validos")
     {
      $where=" estado=2 and historial=0";
      $contar="select count(*) from manual where (estado=2 and historial=0) and id_distrito=$id_distrito";
     }
    elseif($cmd=="historial")
      {
       $where=" historial=1";
       $contar="select count(*) from manual where historial=1 and id_distrito=$id_distrito";
      }

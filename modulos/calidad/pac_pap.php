...
$Author: ferni $
$Revision: 1.17 $
$Date: 2007/03/29 20:39:31 $
*/

require_once("../../config.php");

$id=$parametros['id'];
$ver_mod=$parametros['ver_mod'];
$pac_pap=-1;
switch($_POST['boton'])
{
 ...
 <tr>
  <td>
   ...
   {?>
    ...
    <?if (permisos_check("inicio","mod_log_pac_pap")){?>
    <td>
...
<?if ($ver_mod=="si")$ver_mod="";
else $ver_mod="si";?>

<?$link = encode_link("pac_pap.php",array("pagina"=>"listado","id"=>$id,"ver_mod"=>$ver_mod));?>
<input type="button" name="$" value="$" onclick="document.location.href('<?=$link?>')">
</form>
<center>

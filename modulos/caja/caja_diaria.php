...
variables_form_busqueda("",array("distrito"=>""));
...
$distrito=$_GET['distrito'] or $distrito=$parametros['distrito']  or die("No se indico el distrito");
switch ($distrito) {
 	case 1:
 	$distrito_nombre='San Luis';	
	break;
 	case 2:
...
if($_POST["abrir_caja"]=="Re-abrir caja")
{
 $db->StartTrans();	
 
 //abrimos todas las cajas con el mismo distrito y moneda, cuya fecha es mayor o igual a la caja seleccionada
 $query="update caja set cerrada=0,usuario='".$_ses_user["name"]."',
                         usuario_fecha='".date("Y-m-d H:i:s",mktime())."' 
         where id_distrito=$distrito and id_moneda=$moneda_guardar and fecha>='".Fecha_db($fecha)."' and cerrada=1";
 sql($query,"<br>Error al Re-abrir la caja<br>") or fin_pagina();
 
 put_status("",13,"black");
 
 $db->CompleteTrans();	
}//de if($_POST["abrir_caja"]=="Re-abrir caja")
 ...
 {if($caja_cerrada) {
   ...
   if($cantidad_control_caja==0) {
        ...
        values ($moneda_guardar,$distrito,'".fecha_db($fecha)."',$saldo_actual,'".$_ses_user['name']."','$fecha_creacion',1)";
        ...
        $id_caja=$resultado_set_id_caja->fields['id_caja'];
           ...
           $cerrar_caja="UPDATE caja.caja SET usuario='".$_ses_user['name']."', saldo_total=$saldo_actual,usuario_fecha='$fecha_creacion',cerrada=1  WHERE id_caja=$id_caja";
          $db->Execute($cerrar_caja) or die($db->ErrorMsg().$cerrar_caja);
        }
   $link=encode_link($_SERVER['SCRIPT_NAME'],array("id_caja"=> $id_caja,"pagina"=>"listado","distrito"=>$distrito));
...
<form name="form1" method="post" action="<?= encode_link($_SERVER['SCRIPT_NAME'],array("distrito"=>$distrito)) ?>">
<? 
 ...
 $link_post=encode_link($_SERVER['SCRIPT_NAME'],Array('link'=>'poste','fecha'=>$fe_post,'moneda'=>$moneda,"distrito"=>$distrito));	 
 $link_ant=encode_link($_SERVER['SCRIPT_NAME'],Array('link'=>'ant','fecha'=>$fe_ant,'moneda'=>$moneda,"distrito"=>$distrito));  

 ?>
<table  id=ma class="bordes">
<tr>
<? if (($_POST['Ver'] == 'Ver Caja') || ($parametros['pagina']=='listado') || $parametros['link'] || $ctrl==1) {?> 
 <td width="72" align="left"> <a title="ver caja dia <?=$fe_ant;?>" href='<?=$link_ant?>'> << </a> </td>
 <? } 
 else 
 echo "<td>&nbsp;</td>" ;?>
<td width="536">
...
<TABLE width='98%'>
 <tr>
  <td width="12%"> Moneda:</td>
    <td width="15%"><input type='radio' name='chk_moneda' value='pesos' <?if ($_POST['chk_moneda'] == 'pesos') echo 'checked'; elseif(($parametros['pagina']=='listado')&&($moneda_guardar==1))echo 'checked'; elseif ($parametros ['link'] && $moneda_guardar== 1) echo 'checked'; else echo 'checked' ?>> Pesos </td>
    <td width="16%"><input type='radio' name='chk_moneda' value='dolares' <?if ($_POST['chk_moneda'] == 'dolares') echo 'checked'; elseif(($parametros['pagina']=='listado')&&($moneda_guardar==2))echo 'checked'; elseif ($parametros ['link'] && $moneda_guardar == 2) echo 'checked'?>> Dólares</td>
    <td width="9%">Fecha: </td> <td width="28%"> <input type='text' name='text_fecha'
<?
...
<? if (($_POST['Ver'] == 'Ver Caja') || ($parametros['pagina']=='listado') || $parametros['link'] || $ctrl==1) {?> 
<td width="83" align="right"> <a title="ver caja dia <?=$fe_post?>" href='<?=$link_post?>'> >> </a></td>
<? } 
 else 
 echo "<td>&nbsp;</td>" ;?>
</tr>
...
<br>
   ...
   <?
   if(permisos_check("inicio","permiso_cambiar_caja_cerrada") && $resultados_caja->fields['cerrada']==1)
   {
   ?>
    <input type="submit" name="abrir_caja" value="Re-abrir caja" onclick="if(confirm('Abrir esta caja implica que todas las cajas del mismo distrito y moneda, con fecha posterior a la caja seleccionada, serán abiertas también.\n¿Está seguro que desea continuar?'))return true; else return false;">
   <?
   }
   ?>
</center>
...
ingreso_egreso.comentarios,moneda.simbolo,entidad.nombre as nombre,tipo_cuenta_ingreso.nombre as nombre_cuenta  
from caja.caja join 
caja.ingreso_egreso using(id_caja) 
left join caja.tipo_cuenta_ingreso using(id_cuenta_ingreso)
join licitaciones.entidad using(id_entidad) join licitaciones.moneda 
using(id_moneda) 
...
$sql_egresos="select concepto,plan,caja.fecha,ingreso_egreso.id_ingreso_egreso,ingreso_egreso.item,ingreso_egreso.monto,
ingreso_egreso.comentarios,moneda.simbolo,proveedor.razon_social as nombre 
from caja.caja join caja.ingreso_egreso using(id_caja) 
left join general.tipo_cuenta using(numero_cuenta)
join general.proveedor using(id_proveedor) join 
...
if ($cnr==1)
  ...
  $link = encode_link("ingresos_egresos.php",array("pagina"=>"ingreso","id_ingreso_egreso"=>$res_i->fields['id_ingreso_egreso'],"distrito"=>$distrito));  
  //echo "<tr <?=$atrib>"
 ?>
...
<td align="center"><?=$res_i->fields['nombre_cuenta'] ?></td>
</a>
</tr>
...
<td width="30%"><b>Cuenta: Concepto [Plan] </b></td>
</tr>
<? $cnr=1;
 while (!$res_e->EOF) {
 if ($cnr==1)
  ...
  $link = encode_link("ingresos_egresos.php",array("pagina"=>"egreso","id_ingreso_egreso"=>$res_e->fields['id_ingreso_egreso'],"distrito"=>$distrito));  
 ?>
<tr <?=atrib_tr()?>>
 <a href='<?=$link?>' target="_blank">
...
<td align="center"><?=$res_e->fields['concepto'] ?>&nbsp;[<?=$res_e->fields['plan']?>] </td>
</a>
</tr>
<? $res_e->MoveNext ();} ?>
</table>

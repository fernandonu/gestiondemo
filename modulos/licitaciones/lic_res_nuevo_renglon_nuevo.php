...
$link=encode_link("lic_res_nuevo_renglon_nuevo.php",array('id_licitacion'=>$id_licitacion,'fila'=>$fila));
if ($guardar)
{   $sql = "select nextval('renglon_id_renglon_seq') as id_renglon";
    $id_rec = sql($sql) or fin_pagina();
    $id_renglon=$id_rec->fields['id_renglon'];
	$q="insert into renglon (id_renglon,id_licitacion,codigo_renglon,titulo,cantidad,tipo) values ".
	"($id_renglon,$licitacion,'$renglon','$titulo',$cantidad,'$tipo')";
	if ($db->Execute($q))
	{ $msg="<center><b>SU RENGLON SE GUARDO EXITOSAMENTE</b></center>"; 
	  ...
	  <script>	   
	   window.opener.id_renglon=<?=$id_renglon?>;
	   window.opener.codigo_renglon="<?=$renglon?>";
	   window.opener.cantidad=<?=$cantidad?>;
	   window.opener.titulo="<?=$titulo?>";
	   window.opener.insRow();	  
	   window.close();

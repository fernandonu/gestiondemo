...
*/
$query="select id_licitacion,l.user_login as usuario,l.fecha,o.fecha_entrega,o.lugar_entrega,o.cliente,
	plantilla_pagos.descripcion,moneda.*,razon_social,contactos.nombre as contacto,
	o.cuenta_corriente, o.fecha_facturacion ".
...
if ($f_res->fields["cuenta_corriente"]){
	$pdf->dibujar_planilla(true);
	$pdf->fecha_facturacion(Fecha($f_res->fields["fecha_facturacion"]));
}else $pdf->dibujar_planilla();
...
$pdf->pasa_id_licitacion($f_res->fields['id_licitacion']);
$id_licitacion=$f_res->fields['id_licitacion'];
...
$total=$total + ($datos_prod->fields['precio_unitario'] * $datos_prod->fields['cantidad']);
$datos_prod->MoveNext();
$first=0;
}
...
$pdf->_final($total,$simbolo,$firma1,$firma2,$firma3);
$pdf->Footer();
...
window.location='orden_de_compra_<?=$nro_orden?>.pdf';
</script>
</html
<? }*/
?>
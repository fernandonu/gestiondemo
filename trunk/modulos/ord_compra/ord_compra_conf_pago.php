  ...
  switch($parametros["pagina_pago"]) {
         case "efectivo":
                       ...
                       {$sql="insert into log_ordenes (nro_orden,user_login,fecha,tipo_log)";
                        $sql.=" values ($ordenes_atadas[$i],'$_ses_user[login]','$fecha','Pago orden de compra')";
                       ...
                       {$sql="update orden_de_compra set estado='$estado' where nro_orden=".$ordenes_atadas[$i];
                        $db->execute($sql) or die($sql);
                       	...
                       	$link=encode_link("../ord_compra/ord_compra_pagar.php",array("nro_orden"=>$parametros["nro_orden"],"exito"=>1));
         ...
         case "cheque":
                    ...
                    if (!$error) {
                       ...
                       $sql .= "(IdBanco, FechaEmiCh, FechaVtoCh, FechaPrev, FechaDébCh, NúmeroCh, ImporteCh, IdProv, Comentarios, numero_cuenta) ";
                       $sql .= "VALUES ($banco,'$fe_db','$fv_db',$fp_db,NULL,$numero,$importe,$proveedor,'$comentarios', $nro_cuenta)";
         ...
         case "transferencia":
                      ...
                      if (!$error) {
                               ...
                               //ACA INSERTAN LOS PAGOS DE ORDEN DE COMPRA
...
$link=encode_link("./ord_compra_conf_pago.php",array("pagina"=>$parametros['pagina'],
                                                      "nro_orden"=>$parametros['nro_orden'],
                                                      "valor_dolar"=>$parametros['valor_dolar'],
                                                      "simbolo"=>$simbolo,
                                                      "id_pago"=>$parametros['id_pago'],
...
<tr id="mo">
  <td colspan='2' align="center"> <b> Confirmación de Pagos - Orden de Compra Nro: <? echo $nro_orden; ?> </td>
...
<?
 ...
 case "efectivo":
              ...
              if ($select_tipo){
                ...
                $link=encode_link("../caja/".$parametros['volver'],array("pagina"=>"egreso","pagina_viene"=>"orden_de_compra","nro_orden"=>$nro_orden,
 ...
 case "cheque":
               ...
               if ($proveedor) {
                ...
                $link=encode_link("../bancos/bancos_ing_ch.php",array("pagina"=>"orden_de_compra","pagina_viene"=>"orden_de_compra",
                                                                     "nro_orden"=>$nro_orden,
...
case "transferencia":
      //parte de debitos de la pagina
          ...
          $link=encode_link("../bancos/bancos_ing_deb.php",array("pagina"=>"orden_de_compra",
                                                                 "nro_orden"=>$nro_orden,
                                                                 "valor_dolar"=>$valor_dolar,
                                                                 "id_pago"=>$parametros['id_pago']));
         break;

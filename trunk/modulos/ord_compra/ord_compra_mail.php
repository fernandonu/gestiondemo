...
if ($boton2=="Cancelar")
{
$link=encode_link("ord_compra_listar.php",array('msg'=>"Envio de Email Cancelado"));
...
//		  $mail_header .="\nBcc: ordenesdecompraenviadas@coradir.com.ar";
	   ...
	   $mail_header .= "\n\n".$archivo=chunk_split(base64_encode($pdf->Output("compra", false, true))); 
	      ...
	      {
	      $q="update orden_de_compra set estado='e' where estado='a' AND nro_orden=$clave_valor[1]";
...
$link=encode_link("ord_compra_listar.php",array('msg'=>"Sus Mensajes se enviaron con exito "));
header("location: $link");
die;	
...
$ordenes=$db->Execute($sql) or die($db->ErrorMsg()."<br>".$sql);
?>
<html>
<head>
...
for (var i=0; i < document.all.total_mails.value; i++)
 {
//  alert(eval('document.all.email_'+i.toString()+'.value'));

...
/*if ($_POST['carga']) {
     $sel="select fecha,id_info_rma from stock.log_info_rma where tipo_log='Creacion'";
     $res_sel=sql($sel,"No se pudo recuperar los log")or fin_pagina();
     while(!$res_sel->EOF)
     {
     	$idrma=$res_sel->fields['id_info_rma'];
     	$feccrea=$res_sel->fields['fecha'];
     	$cons="update stock.info_rma set fecha_creacion='$feccrea' where id_info_rma=$idrma";
        sql($cons,"No se pudo actualizar el rma") or fin_pagina();
     	$res_sel->MoveNext();
     }
}*/
...
$pagina_oc=$parametros["pagina_oc"] or $pagina_oc=$_POST['pagina_oc'];
 ...
 if ($_POST["form_busqueda"])  {
     if(!$_POST["sin_monitor"]){
         $_POST["sin_monitor"]=-1;
     }
   }	

variables_form_busqueda("listar_rma",array("filtro_atendido_por"=>-1,"tipo_producto"=>"todos","sin_monitor"=>""));
...
select * from (
    select info_rma.cantidad, en_stock.id_prod_esp,en_stock.ubicacion,estado_rma.nombre_corto,producto_especifico.descripcion,
          ...
          info_rma.id_movimiento_material as pm,texto
     from stock.en_stock
     ...
     left join (select comentario.id_info_rma,texto 
                from (select max(id_comentario_rma) as id_comentario_rma,id_info_rma
                        from stock.comentario_rma group by id_info_rma order by id_info_rma) as comentario
                 join stock.comentario_rma using (id_comentario_rma)) as comentarios 
     using (id_info_rma) 
     ) as rma
     "; 
...
$where="$where $group";
  ...
  // } //si cmd  real
   $contar="buscar";
   $link=encode_link("listar_rma.php",array("id_deposito"=>$id_deposito,"deposito"=>$deposito,"cmd"=>$cmd,'keyword'=>$keyword));
...
<form name="form1" method=POST action="listar_rma.php">
  ...
  <td align=Center  colspan=4>
   <table width=100% align=center class='bordes' bgcolor=<?=$bgcolor3?>>
      <tr>
        ...
        switch ($cmd)
	       {case "tran": $cons.=" where nombre_corto='T'";
	              break;
	        case "cor":  $cons.=" where nombre_corto='C'";
	              break;
	        case "prov": $cons.=" where (nombre_corto='P') ";
	              break;
	        case "real": $cons.= " where (nombre_corto <> 'B' and nombre_corto <> 'E')";
	              break;
            case "historial": $cons.= " where (nombre_corto='B')";
                       break;
	   };
      ...
      <b>
      <font color=black>
       Monto: <?echo "u\$s&nbsp;".formato_money($mont);?>
      </font>
       </b>
         </td>
         <td align="left">
             <?
   $ref1 = encode_link("stock_rma.php",array("pagina"=>1));
   ...
   if(permisos_check("inicio","permiso_cargar_nuevo_rma"))
   {
   ?>
      <input type="button" name="nuevo" value="Nuevo RMA" onclick="document.location='<?=$ref1?>'">
   <?
   }//de if(permisos_check("inicio","permiso_cargar_nuevo_rma"))
   ?>
      <input type="button" name="informe" value="Informe" title='Informe' onclick="window.open('<?=$link_info?>','','')">
...
<?
 ...
 <tr>
   ...
   $link_vista = encode_link("configurar_vista.php",array("deposito"=>"RMA"))
   ?>
   <input type="button" name="configurar" value="Configurar Vista"  onclick="window.open('<?=$link_vista?>');">
   ...
   if ($cmd=='tran') { ?>
      &nbsp;
      <input type="checkbox" name='sin_monitor' value="1" class="estilos_check" <?if($sin_monitor==1)echo "checked";?>>
      <b>Filtrar Monitor </b>
    <? } 
   
   
   if ($_ses_user["login"]=="fernando") {
   ?>
   <input type="submit"  name="actualizar_movimientos" value="Actualizar MM">   
   <?
   }
   ?>
   </td>
  ...
  /////////cambie la consulta sacando * Quique///////
  //$sql = "select * from configurar_vista where id_usuario=$id_usuario";
  $sql = "select campo,ver from configurar_vista where id_usuario=$id_usuario and deposito = 'RMA'";
  $rs = sql($sql) or fin_pagina();
...
//		 if ($cmd=="real" || $cmd=="tran" || $cmd=="cor" || $cmd=="prov" || $cmd="historial") {
       ...
       if ($resultado->fields["texto"]) {
			$title_obs=$resultado->fields["texto"];

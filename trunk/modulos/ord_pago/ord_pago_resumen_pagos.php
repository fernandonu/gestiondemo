...
//obtengo los datos
$sql="select comentario_pagos from orden_de_compra where nro_orden=$nro_orden";
...
<tr id="mo">
    <td colspan='<?=$colspan; ?>' align="center">
        <b>Resumen de los Pagos - Orden de Pago Nro: <?=$nro_orden;?>
    </td>
</tr>
<tr class="sub_tabla">
...
if (!($moneda)) {
   ...
   if ($resultado->fields["id_ingreso_egreso"]){
                                     ...
                                     echo "<td>Efectivo       </td>";
                                       $link_caja = encode_link("../caja/ingresos_egresos.php",Array ("id_ingreso_egreso" => $nro_ingreso, "pagina" => "egreso", "distrito" => 1,"pagina_viene"=>'pagos_op',"cmd"=>'egresos' ));           
                                      echo "<td align='right' onclick = window.open('$link_caja','','left=100,top=50,height=400,width=700'); title='Ver este egreso' style='cursor:hand; color:blue;'> $nro_ingreso </td>";
                                     echo "<td align='right'> $simbolo $monto      </td>";
                                     echo "<td align='center'> $usuario    </td>";
                                     echo "<td align='right'> $fecha       </td>";
                                     }
  if ($resultado->fields["iddébito"])  {
        ...
        if ($resultado->fields["id_ingreso_egreso"]){
                                     $nro_ingreso=$resultado->fields['id_ingreso_egreso'];
                                     echo "<td>Efectivo       </td>";
                                      $link_caja = encode_link("../caja/ingresos_egresos.php",Array ("id_ingreso_egreso" => $nro_ingreso, "pagina" => "egreso", "distrito" => 1,"pagina_viene"=>'pagos_oc',"cmd"=>'egresos' ));           
                                      echo "<td align='right' onclick = window.open('$link_caja','','left=100,top=50,height=400,width=700'); title='Ver este egreso' style='cursor:hand; color:blue;'> $nro_ingreso </td>";
                                     echo "<td align='right'> $simbolo $monto       </td>";
                                     echo "<td align='right'> $valor_dolar </td>";
                                     echo "<td align='right'> \$ $monto_pagado </td>";
      ...
      if ($resultado->fields["iddébito"])  {
                                     $nro_debito=$resultado->fields['iddébito'];
                                      echo "<td>Transferencia       </td>";
                                      $link_tr = encode_link("../bancos/bancos_movi_debitos.php",Array("id_debito"=>$nro_debito,"pagina"=>"mail","disabled"=>"disabled" ));
                                      echo "<td align='right' onclick = window.open('$link_tr','','left=100,top=50,height=400,width=700'); title='Ver esta transferencia' style='cursor:hand; color:blue;'> $nro_debito </td>";
                                     echo "<td align='right'> $simbolo $monto      </td>";
                                     echo "<td align='right'> $valor_dolar </td>";
                                     echo "<td align='right'> $simbolo $monto_pagado </td>";
                                     echo "<td align='center'> $usuario    </td>";
                                     echo "<td align='right'> $fecha       </td>";
                                       }
    } //del else
...
?>
<tr>
 <td colspan='<?=$colspan;?>' class="sub_tabla">
 &nbsp;
 </td>
</tr>
<tr class="tablaEnc">
...
<tr>
 ...
 if ($parametros['pagina']!="ord_compra" || $parametos['ver']=="pagos") {
       //$link=encode_link("../ord_compra/ord_compra_pagar.php",array("nro_orden"=>$parametros["nro_orden"]));
       $link=encode_link("../ord_pago/ord_pago_listar.php",array());?>

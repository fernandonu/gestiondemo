...
$Author: mari $
$Revision: 1.4 $
$Date: 2007/01/05 18:17:24 $
*/
require_once("../../config.php");
require_once("fns.php");


//en los arreglos $datos_forma, $datos_nc,$datos_pagos se guardan datos de la orden de Pago
...
$nro_orden_orig=$_POST['nro_orden_orig'];
...
if($_POST['boton']=="Aceptar")
{
 ...
 if($msg=="")
 {
  ...
  {
   $link_padre=encode_link("ord_pago.php",array("nro_orden"=>$_POST['nro_orden_padre']));
   echo "window.opener.location.href='$link_padre';";
  }
 if($pagina=="pago_multiple" &&$_POST['volver_a_padre']!=1)
 {
  $link_pagar=encode_link("./ord_pago_pagar.php",array("nro_orden"=>$nro_orden[0]));
 ...
 for($i=0;$i<$cant_pagos;$i++)
 {
  ...
  {//antes de desatar cheuque, caja o débito, traemos los datos de ese pago, para despues usarlos
   ...
   {$query="update orden_de_compra set estado='$estado_nuevo' where nro_orden=".$nro_orden[$r];
    sql($query,"<br>Error al modificar estado de la Orden de Pago<br>") or fin_pagina();
   }
  ...
  elseif($datos_pago_borrado->fields['númeroch']!="")
  {
   ...
   {
   	$asunto="Se Deshizo un pago de la Orden de Pago: ".$nro_orden[0]."\n";
   	$texto="La Orden de Pago ".$nro_orden[0]." se volvió a estado enviada.\n";
   	$texto.="La Orden de Pago que se habia realizado se desató de dicho Pago, pero quedó cargado en el sistema.";
   }
   else
   {
   	$asunto="Se Deshizo un pago de las Ordenes de Pago: ".join(", ",$nro_orden)."\n";
   	$texto.="Las siguientes Ordenes de Pago se volvieron a estado enviada por formar parte de un mismo Pago Múltiple:\n".join(", ",$nro_orden)."\n";
   	$texto.="El pago que se habia realizado se desató de dichas Ordenes de Pago, pero quedó cargado en el sistema.";
   }
   ...
   $msg="<center><b>El pago ".$nro_orden[0]." se deshizo con éxito</b></center>";
   $recargar_despago=1;
...
if($_POST['desatar']=="Separar Ordenes de Pago")
{//Separa las ordenes de Pago de un pago multiple a formas
 //de pago separadas (cheque a 30 dias).
 guardar_forma_pago_ant('separar pago múltiple'); // para los de modificaciones
...
{ $link=encode_link($_SERVER['SCRIPT_NAME'],array('reload'=>0,"armando"=>$parametros['armando'],"presupuesto"=>$parametros['presupuesto']));
...
elseif(($estado_orden=='a' || $estado_orden=='e' || $estado_orden=='m' || $estado_orden=='d')&& $pago_especial)
{$permiso="";
 $mostrar_montos=1;
 $boton_value="Cancelar";
 $guardar_pagos="si";
...
//La variable total contiene el total de la orden de Pago
...
ESTE TOTAL SE CALCULA SIEMPRE EN LA MONEDA DE LA ORDEN DE Pago
...
// alert('total pagar '+total_oc+' total '+total_notas_credito+' monto '+monto);
 ...
 {alert('El monto de la Nota de crédito supera el monto a pagar de las ordenes de Pago');
  objeto.checked=false;
  return 0;
 }
 else
...
{
 if(confirm('Al despagar una pago, se borrarán egresos de caja o se anularán cheques cargados, que hayan sido usados para pagar ese Pago\n¿Está seguro que desea despagar este pago?'))
...
if($pagina=="pago_multiple")
 {if($cant_pm_orden>1)
  ...
  $link=encode_link($_SERVER['SCRIPT_NAME'],array("pagina_viene"=>$pagina,"ordenes_multiple"=>$ordenes,"armando"=>$parametros['armando']));
 }
 else
  $link=encode_link($_SERVER['SCRIPT_NAME'],array("pagina_viene"=>$pagina,"armando"=>$parametros['armando']));
...
<?
$link1=encode_link('ord_pago_pagoant.php',array("id_log_pago"=>$res_log_pagos->fields['id_log_pago'],"ord_pago"=>$nro_orden));
...
<script>total_oc=<?if($total_a_pagar!="")echo $total_a_pagar; elseif($nro_orden[0]!="-1") echo monto_a_pagar($nro_orden[0]);else echo 0;?>;</script>
<br>

<?
//Se traen las notas de creditos disponibles (pendientes o reservadas)
//para el proveedor de la orden de Pago.
...
<table border=1 width="80%" cellspacing=0 cellpadding=5 align="center">
<tr>
           <td style=<?="border:$bgcolor3;"?>  align="center" class="tablaEnc" colspan=5>
...
while(!$notas_credito->EOF)
{
 ...
 // + las que esten asociadas a esta orden de Pago
...
<tr>
 <td align="center" colspan="2">
  ...
  <?if(($mostrar_montos && $permiso=="" && $estado_orden!="")||$recargar_despago)
   ...
   //para volver a pagar orden cuando se llama del menu pagos/cobros
	$link_cancelar=encode_link("ord_pago_pagar.php",array("nro_orden"=>$nro_orden_orig));
      ?>
  <input type="hidden" name="recargar_padre" value="<?=$recargar_padre?>">
  <input type="hidden" name="nro_orden_orig" value="<?=$nro_orden_orig //en caso de que se haga un autosubmit ?>" />
  ...
  <input type="button"   name="boton"  value="<?=$boton_value?>" onclick="if(document.all.cambios_forma.value=='si')
                                                                          {var x=confirm('Se han realizado cambios al formulario. ¿Está seguro que desea salir sin guardar los cambios?');
                                                                           if(x)
                                                                           {
                                                                            ...
                                                                            else	location.href='<?=$link_cancelar?>';
                                                                           }
                                                                          }
                                                                          else
                                                                          {
...
{?>
 <table width="80%" align="center">
  <tr><td align="right">
   ...
   ?>
   <input type="submit" name="desatar" <?=$permiso?> value="Separar Ordenes de Pago" title="Separa las ordenes de Pago del pago multiple a formas de pago separadas (cheque a 30 dias)" onclick="return confirm('Se van a cambiar las formas de pago de las ordenes de Pago:<?=$confirm_text?>. ¿Está seguro que desea continuar?')">
  </td></tr>
 </table>
<?
}

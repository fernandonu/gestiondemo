...
<?
  ...
  elseif($cmd=="ingresos")
	$in="ingreso";
	$link=encode_link("egresos_bsas.php",array("id_ingreso_egreso"=> $parametros["id_ingreso_egreso"],"pagina"=>$in,"cuotas"=>$parametros['cuotas'],
	                  "cuentas"=>$parametros['cuentas'],"cantidad_cuentas"=>$parametros['cantidad_cuentas']));


   if ($parametros["pagina_viene"]=="orden_de_compra")
	 {
	  $link=encode_link("./egresos_bsas.php",
...
elseif($_POST['filtro']=="todos")
 ...
 if($cmd=="egresos")
 {$titulo_principal="EGRESOS - CAJA BUENOS AIRES";
  $param1="EGRESO";
  $param2="PROVEEDORES";
  $param3="#BA0105";
 }
 elseif($cmd="ingresos")
 {$titulo_principal="INGRESOS - CAJA BUENOS AIRES";
  $param1="INGRESO";
  $param2="CLIENTES";
  $param3=$bgcolor1;
 }
...
echo "<br>";
 show_titulo($titulo_principal,"Buenos Aires");
 ...
 <td width='40%'>
  <?
   if($cmd=="egresos") {
	...
	generar_parte_derecha("egreso","Buenos Aires");
	}
   elseif($cmd=="ingresos"){
	tabla_filtros_nombres("","ingreso");
	generar_parte_derecha("ingreso","Buenos Aires");
	}
...
<!-- <input type='submit' name='Cancelar' value='Cancelar' style='cursor:hand' title='Presione aqui para cancelar'>-->
</center>
</form>
<?=fin_pagina();?>

	...
	<tr>
	 ...
	 <?
	  $sql_dist="select id_distrito,nombre
             ...
             where nombre ilike 'Buenos Aires%' or nombre ilike '%San Luis%'";
      $res_dist=sql($sql_dist,"$sql_dist") or fin_pagina();
    ?>
	 <td>
	  <select name="ubicacion">
	   <? while (!$res_dist->EOF) {?>
	        <option value=<?=$res_dist->fields['id_distrito']?> <?if ($res_dist->fields['id_distrito'] == $ubicacion) echo 'selected'?>> 
	           <?=$res_dist->fields['nombre']?></option>
	   <?$res_dist->MoveNext();}?>
	   </select>
	 </td>
	</tr>

...
if ($_POST["atar_remito"]){
	...
	if ($_POST['id_items']){		
		...
		$db->Execute($sql) or die($db->ErrorMsg()."<br>".$sql);?>
 		...
 		<br>
		<div align="center">
			<font color="Green" size="3"><b>El Remito se Ato con Exito</b></font>
		</div>
		<br>
 		<?
	}
		...
		<br>
		<div align="center">
			<font color="Red" size="3"><b>Debe Seleccionar un Items del Remito</b></font>
		</div>
		<br>
	<?}
}//del if ($_POST["atar_remito"])
...
<script>
	function confirmaAtado(){
		...
		if (radio.length==undefined){
			...
			if (hidden_conf=="1"){
				res = confirm("Este Remito ya se Encuentra Vinculado al Id: "+ hidden_conf_lic +" Cliente: "+ hidden_conf_nombre +",   Esta Seguro que Desea Atar?");
				if (res==true){
					return true;
				}
				else{
...
<?
           variables_form_busqueda("atar_remito");
    ...
    $sql="select id_remito, nro_remito, cliente, direccion, fecha_remito 
    		from facturacion.remitos";

?>
    <table width=100% align=center class=bordes>
       <tr>
          <td colspan=5 id=mo>Elija el Remito que desea Atar</td>
          </td>
       </tr>
      <tr>
      ...
      <tr id=ma>
       ...
       <tr id=mo>
          <td width=1%>&nbsp;</td>
          <td><a id=mo href='<?=encode_link("atar_remito.php",array("sort"=>"1","up"=>$up))?>'>Número de Remito</a></td>
          <td><a id=mo href='<?=encode_link("atar_remito.php",array("sort"=>"2","up"=>$up))?>'>Cliente</a></td>
          <td><a id=mo href='<?=encode_link("atar_remito.php",array("sort"=>"3","up"=>$up))?>'>Dirección</a></td>
          <td><a id=mo href='<?=encode_link("atar_remito.php",array("sort"=>"4","up"=>$up))?>'>Fecha del Remito</a></td>
       </tr>
       <?
       $cantidad=$result->recordcount();
       $cont_hidden_confirm=0;
       for($i=0;$i<$cantidad;$i++){
	       $id_tabla="tabla_remito_".$result->fields["id_remito"];
	       ...
	       <tr <?=atrib_tr()?> >
	          <td>
	           <input type=checkbox name=check_id_remito value="<?=$result->fields["id_remito"]?>" onclick="<?=$onclick_check?>" class="estilos_check">
	          </td>
	          <td><?=$result->fields["nro_remito"]?></td>
	          <td><?=$result->fields["cliente"]?></td>
	          <td><?=$result->fields["direccion"]?></td>
	          <td><?=Fecha($result->fields["fecha_remito"]);?></td>
	        </tr>
	        ...
	        <tr>
	          <td colspan=6>
	                  ...
	                  <?
	                  $sql=" select facturacion.items_remito.*, id_licitacion, nombre
	                         ...
	                         where id_remito=". $result->fields["id_remito"]." order by id_item";
	                           ...
	                           <tr id=ma>
	                            ...
	                            for ($y=0;$y<$cantidad_items;$y++){?>
		                            <tr <?=atrib_tr()?>>
		                                 ...
		                                 <td><?=$result_items->fields["cant_prod"]?></td>
       ...
       <tr>
          <td colspan=6 align=center>
            <input type=submit name='atar_remito' value='Atar Remito' onclick="return confirmaAtado()">
            &nbsp;

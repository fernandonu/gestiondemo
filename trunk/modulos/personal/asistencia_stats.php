...
*/	
	...
	if ((!$empleado)&&($id_legajo!=-1)){
		$consulta="select nombre, apellido, hr_entra, hr_sale from legajos where id_legajo=".$id_legajo;
		...
		$consulta="select u.nombre, u.apellido, l.hr_entra, l.hr_sale from usuarios u left join legajos l using (id_usuario) where id_usuario=".$id_usuario;
...
<form action='asistencia_stats.php' method='post' name='form1'>
	...
	<?if(permisos_check("inicio", "nuevo_ingreso_personal_asistencia")){?>

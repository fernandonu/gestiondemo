  ...
  $id_afjp=$_POST['leg_afjp'];
	...
	if (!$error) {
		if ($_POST["leg_nuevo"] == "1") {
			...
			$sql .= "tipo_jub,idbanco,id_tarea,id_calificacion";
			$sql .=",id_afjp";
			...
			$sql .= "$tipo_jub,$idbanco,$id_tarea,$id_calif";
			$sql.=",$id_afjp";
		...
		elseif ($_POST["leg_nuevo"] == "0") {
			...
			$sql .= "idbanco=$idbanco,id_tarea=$id_tarea,id_calificacion=$id_calif ";
	...
	<tr><td colspan="2">
	  ...
	  <table>
        ...
        <tr><td colspan="2">
        	...
        	<?
        		$rta_consulta=sql("select * from licitaciones.distrito", "No se pudo conseguir la lista de distritos") or fin_pagina();
...
?>
	  ...
	  <td valign="top"><table>
	         <tr><td><b>Banco:</b></td></tr>
	         <? $q_bancos="select idbanco, nombrebanco from bancos.tipo_banco where not suc_destino is null";
	            $res_q_bancos=sql($q_bancos, "Error al traer los datos d elos bancos") or fin_pagina();
	            $cant_bancos=$res_q_bancos->RecordCount();
	         ?>
	         <tr><td>
	             <select name="leg_banco">
	             <? for($i=0;$i<$cant_bancos;$i++) {
	                  $id_banco_res=$res_q_bancos->fields['idbanco'];
	             ?>
	             <option value='<?=$id_banco_res?>'
	             <?if ($idbanco==$id_banco_res) echo "selected"?>>
	              <?=$res_q_bancos->fields['nombrebanco'];?>
	             </option>
	             <? $res_q_bancos->MoveNext();} ?>
	             </select>
	         </td></tr>
					...
					<td width="100%" colspan="2">
						<table width="100%">
							<tr id="mo">
								<td align="center">
									Capacitaciones
									<?if (permisos_check("inicio", "boton_legajo_ver_notas")){?>
...
<tr>
	...
	?>
	   ...
	   <? $i=1;
        ...
        $link1="window.open('$link','','top=50, left=170, width=800, height=600, scrollbars=1, status=1,directories=0');";
        if (permisos_check("inicio","permiso_nuevo_sumario"))
        {
        ?>
	    <tr>
...
/**********************Fin Tabla Sumario************************/	
	...
	if ($es_evaluador or $nuevo or $usuario == $_ses_user["id"] || permisos_check("inicio","guardar_legajo")) {
	...
	//if ($parametros["cmd"] == "modificar") {
	if(permisos_check("inicio","liq_sueldo")) {
	 //echo "<input style='width:160' type='submit' name='leg_guardar' value='Guardar Cambios'>";
     $link_leg=encode_link('../liquidacion_sueldos/liq_sueldo.php', array("id"=>$id,"pagina"=>"modificar_legajo.php"));
     echo "&nbsp;&nbsp;&nbsp;<input style='width:160' type='button' name='leq_sueldo' value='Liquidación de Sueldo' onClick=\"document.location='$link_leg';\">";
